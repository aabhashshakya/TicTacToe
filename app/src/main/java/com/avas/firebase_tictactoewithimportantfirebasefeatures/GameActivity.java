...
public class GameActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        choosePlayerScreen.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent = new Intent(GameActivity.this, MatchmakerActivity.class);
    ...
    private void addUserChangeListener() {
        ...
        mFirebaseDatabase.child(userId).addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                ...
                playerText.setText(loggedInUser.getEmail() + " vs. " + loggedInUser.getOpponentEmail());

                if (user.getMyGame().gameInProgress == false && user.isCurrentlyPlaying() == true) {
                    if (loggedInUser.getMyGame().currentLetter.equals("X")) {
                        // populates the gameBoard array, swapping x with y
                        gameBoard[user.getMyGame().currentMove] = 2;
                        printMove(user.getMyGame().currentMove, "O");
                    } else {
                        gameBoard[user.getMyGame().currentMove] = 1;
                        printMove(user.getMyGame().currentMove, "X");
                    }
                    ...
                    mFirebaseDatabase.child(loggedInUser.getMyID()).child("myGame").child("gameInProgress").setValue(false);
                    mFirebaseDatabase.child(loggedInUser.getMyID()).child("currentlyPlaying").setValue(false);

                    mFirebaseDatabase.child(loggedInUser.getOpponentID()).child("myGame").child("gameInProgress").setValue(false);
                    mFirebaseDatabase.child(loggedInUser.getOpponentID()).child("currentlyPlaying").setValue(false);
                    ...
                    mFirebaseDatabase.child(loggedInUser.getOpponentID()).child("opponentEmail").setValue("");
                    mFirebaseDatabase.child(loggedInUser.getOpponentID()).child("opponentID").setValue("");

                    Toast.makeText(GameActivity.this, "GAME IS OVER", Toast.LENGTH_LONG).show();
                }
                // if my turn, draw board and enable buttons
                else if (user.getMyGame().currentTurn.equals(user.getMyID())) {
                    turn.setText("Your turn to make a move");
                    // make sure there was a move

                    if (user.getMyGame().currentMove != -1) {
                        if (loggedInUser.getMyGame().currentLetter.equals("X")) {
